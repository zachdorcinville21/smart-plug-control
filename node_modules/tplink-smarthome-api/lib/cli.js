        ...
        getClient()
            ...
            if (sysInfo) {
                console.dir(device.sysInfo, {
                    colors: commander_1.program.opts().color === 'on',
                    depth: 10,
                });
            }
        });
...
async function send(host, port, payload) {
    try {
        ...
        console.dir(data, { colors: commander_1.program.opts().color === 'on', depth: 10 });
    }
    catch (err) {
        outputError(err);
    }
...
async function sendCommand(host, port, childId, payload) {
    try {
        ...
        console.log('response:');
        console.dir(results, { colors: commander_1.program.opts().color === 'on', depth: 10 });
    }
    catch (err) {
        outputError(err);
    }
...
async function details(host, port) {
    try {
        ...
        console.dir({
            ...
            mac: device.mac,
        }, { colors: commander_1.program.opts().color === 'on', depth: 10 });
    }
    catch (err) {
        outputError(err);
    }
...
commander_1.program
    ...
    search(options.sysinfo, options.breakoutChildren || false, commander_1.program.opts().timeout, options.broadcast, paramsObj);
});
commander_1.program
...
commander_1.program.command('details <host>').action((host) => {
    const [hostOnly, port] = host.split(':');
...
for (const command of commandSetup) {
    ...
    cmd.action((...args) => {
        ...
        sendCommandDynamic(hostOnly, port, command.name, commandParams, sendOptions, childId);
    });
}
commander_1.program
